...
void signal_handler(signal_code sig)
{
	switch(sig)
	{
		case 600:
			 ...
			 break;
		default:
			printf("Incorrect Signal - CarDoor\n");
	}
	_os_rte();
}
...
{	
   ...
   if((errno=_os_alarm_cycle(&alarmID, wakeUpCall, timeToDelay)) != 0)
	...
	printf("Car Door Process Started\n");

	
	while(carData->carDoorClose == FALSE)
	{
		...
		_os_send(pidMem->childrenPID[3], 500);
		...
		*/ 
		...
		/*printf("Car Door - unlink event\n")	;		*/
	}
   /*	printf("Car door is closed\n");*/
  	_os_send(pidMem->childrenPID[6], 1500);
 	_os_unlink(&mod_head);
}
